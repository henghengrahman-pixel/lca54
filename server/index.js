...
function setAdminCookie(res,token){
  res.setHeader('Set-Cookie',`lc_admin=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=43200; Secure`);
}
...
app.post('/login',(req,res)=>{
  ...
  setAdminCookie(res,token); store.audit(id,'LOGIN_OK_FORM');
...
app.post('/api/auth/login',(req,res)=>{
  ...
  store.audit(id,'LOGIN_OK');res.json({ok:true,token,id});
});
...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'livechat-mischat-monitor',version:'1.0.4',admin_id_set:!!ADMIN_ID,admin_password_set:!!ADMIN_PASSWORD,session_secret_set:SESSION_SECRET.length>=24,extension_key_set:EXT_KEY.length>=24,time:new Date().toISOString()}));
...
app.listen(PORT,()=>{
  console.log(`LiveChat Mischat Monitor v1.0.4 running on :${PORT}`);
