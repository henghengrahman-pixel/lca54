...
class Store {
  constructor(dataDir){
    ...
    this.db = new DatabaseSync(path.join(dataDir, 'monitor.db'));
  ...
  migrate(){
    this.db.exec(`
      ...
      DROP INDEX IF EXISTS idx_scan_active_date;
      CREATE UNIQUE INDEX IF NOT EXISTS idx_scan_active_date ON scans(target_date) WHERE status IN ('QUEUED','RUNNING');
  ...
  createScan(targetDate, actor='admin'){
    ...
    try{
      ...
      const raced=this.db.prepare("SELECT * FROM scans WHERE target_date=? AND status IN ('QUEUED','RUNNING') ORDER BY created_at DESC LIMIT 1").get(targetDate);
      if(raced) return {scan:raced, existed:true};
      throw e;
    }
  ...
  scans(limit=50){ return this.db.prepare('SELECT * FROM scans ORDER BY created_at DESC LIMIT ?').all(Math.max(1,Math.min(Number(limit)||50,500))); }
  claimScan(deviceId){
    ...
    try{
      ...
      if(!s){ this.db.exec('COMMIT;'); return null; }
      const lockUntil=new Date(Date.now()+60_000).toISOString();
      this.db.prepare("UPDATE scans SET status='RUNNING',device_id=?,started_at=COALESCE(started_at,?),lock_until=? WHERE id=? AND status='QUEUED'").run(deviceId,n,lockUntil,s.id);
      const claimed=this.scan(s.id);
      this.db.exec('COMMIT;');
      return claimed;
    }catch(e){
  ...
  upsertResult(r){
    const n=nowIso();
    this.db.prepare(`INSERT INTO results(scan_id,target_date,chat_id,fingerprint,user_id,agent,last_customer_text,last_customer_at,status,archive_url,screenshot_path,created_at,updated_at)
      VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
      ...
      .run(r.scan_id,r.target_date,r.chat_id,r.fingerprint,r.user_id,r.agent,r.last_customer_text,r.last_customer_at,r.status,r.archive_url,r.screenshot_path,n,n);
  ...
  recountScan(sid){
    ...
    const counts={NORMAL:0,MISCHAT:0,ERROR:0}; rows.forEach(r=>counts[r.status]=Number(r.c));
    const checked=rows.reduce((a,r)=>a+Number(r.c),0);
  ...
  pendingTelegram(limit=20){ return this.db.prepare("SELECT * FROM results WHERE status='MISCHAT' AND telegram_status IN ('NOT_SENT','RETRY') ORDER BY created_at ASC LIMIT ?").all(Math.max(1,Math.min(Number(limit)||20,100))); }
  ...
  dashboardSummary(){
    ...
    const byAgent=this.db.prepare("SELECT COALESCE(agent,'Unknown') agent,COUNT(*) count FROM results WHERE target_date=? AND status='MISCHAT' GROUP BY agent ORDER BY count DESC").all(yesterday).map(x=>({...x,count:Number(x.count)}));
